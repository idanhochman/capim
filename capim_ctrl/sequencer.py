...
boundaries (the confidence-driven scheduling that is CAPIM's contribution):
  ...
  1. prune_tree(step, sigma_th) — the live σ_th confidence gate.  A draft node is
     ...
     path) is >= sigma_th.  That score decreases monotonically with depth, so a
     failing node guarantees all its descendants fail too and one independent pass
     suffices.  sigma_th = -inf disables the gate, which is how a trace already gated
     inside the GPU drafting loop is replayed as-is; a finite sigma_th applies the
     gate at re-cost time.
  ...
  2. route(mu, mu_th) — the verify execution plan, binary on the gated tree size mu
     (PAPI-style at RLP=1):
       ...
       mu >= mu_th -> concurrent NPU||PIM column-split makespan, attention PIM-pinned
...
cost_forward_pass call.  It never touches Device/cost, which keeps policy and
mechanism separated.
"""
...
def prune_tree(step: DecodeStep, sigma_th: float) -> List[TokenNode]:
    ...
    sigma_th = -inf disables the gate and returns the full tree, which is the path
    taken when replaying an already-gated trace.
    """
    if not step.nodes:
        return []
...
def route(mu: int, mu_th: int, concurrent_verify: bool = True) -> Route:
    ...
    mu >= mu_th : concurrent NPU||PIM FC column-split makespan, attention PIM-pinned.
                  `concurrent_verify=False` falls back to an all-NPU-FC sequential
                  big-tree route, kept for ablation.
    """
    if mu < mu_th:
