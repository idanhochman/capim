...
The retrospective, content-blind selector that is the LP-Spec baseline's
counterpart to CAPIM's live σ_th gate.  Grounded in two papers:
...
def k_pred_map(step: DecodeStep) -> Dict[Pos, int]:
...
def assert_sibling_rank_order(step: DecodeStep) -> None:
    """Assert siblings appear in ascending layer_idx == ascending derived rank, so
    deriving k_pred from parent_idx is valid.
    """
...
class DTPHist:
    ...
    assumption; an assumption-free cross-check).
    """
    ...
    def update(self, step: DecodeStep, kp: Dict[Pos, int] = None,
               pp: Dict[Pos, Pos] = None) -> None:
        """Fold one step's observations in, counting a node only if reachable (its
        parent was accepted; depth-0 nodes hang off the always-accepted root).
        """
        if kp is None:
            kp = k_pred_map(step)
        if pp is None:
...
def score_nodes(step: DecodeStep, hist: DTPHist, kp: Dict[Pos, int] = None,
...
def effective_accept(step: DecodeStep, kept: set) -> int:
    ...
    non-decreasing in L and ≤ the measured full-tree accept.  Mirror of CAPIM's
    `_effective_accept`.
    """
...
def select_kept(step: DecodeStep, t: int, L: int, selection: str, hist: DTPHist,
                kp: Dict[Pos, int] = None, pp: Dict[Pos, Pos] = None) -> set:
